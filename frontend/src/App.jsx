...
function serverToTrack(song) {
  return {
    ...
    url: `${API_URL}/songs/${encodeURIComponent(song.filename).replace(/%2F/g, '/')}`,
...
export default function App() {
  ...
  // ===== FETCH SONGS FROM SERVER (progresivo) =====
  const fetchSongsFromServer = useCallback(async () => {
    try {
      setLoading(true);
      setTracks([]);
      setAllTracksLoaded(false);
      const r=await fetch(`${API_URL}/api/songs?limit=${PAGE_SIZE}&offset=0`);
      if(!r.ok)throw Error('Error');
      const d=await r.json();
      const fb=d.songs.map(serverToTrack);
      setTracks(fb);setCurrentPage(0);setHasMore(d.hasMore);setLoading(false);
      if(d.hasMore){
        let at=[...fb];
        for(let pg=1;pg<=5;pg++){
          const r2=await fetch(`${API_URL}/api/songs?limit=${PAGE_SIZE}&offset=${pg*PAGE_SIZE}`);
          if(!r2.ok)break;
          const pd=await r2.json();
          const bt=pd.songs.map(serverToTrack);
          at=[...at,...bt];setTracks([...at]);setCurrentPage(pg);
          setHasMore(pd.hasMore);
          if(!pd.hasMore){setAllTracksLoaded(true);break}
        }
      }else setAllTracksLoaded(true)
    }catch(err){console.error(err);setLoading(false)}
  },[]);
  // ===== CARGAR MÁS (scroll infinito) =====
  const loadMoreSongs=useCallback(async()=>{if(isLoadingMore||!hasMore)return;setIsLoadingMore(true);
    try{
      const np=currentPage+1;
      const r=await fetch(`${API_URL}/api/songs?limit=${PAGE_SIZE}&offset=${np*PAGE_SIZE}`);
      if(!r.ok)throw Error('Error');
      const d=await r.json();
      const bt=d.songs.map(serverToTrack);
      setTracks(prev=>{const e=new Set(prev.map(t=>t.id));return[...prev,...bt.filter(t=>!e.has(t.id))]});
      setCurrentPage(np);setHasMore(d.hasMore);
      if(!d.hasMore)setAllTracksLoaded(true)
    }catch(err){console.error(err)}finally{setIsLoadingMore(false)}
  },[isLoadingMore,hasMore,currentPage]);


  // ===== REFRESH FROM SERVER =====
  const handleRefresh=useCallback(async()=>{try{setLoading(true);
      await fetch(`${API_URL}/api/sync-db`,{method:'POST'});
      setTracks([]);setAllTracksLoaded(false);
      const r=await fetch(`${API_URL}/api/songs?limit=${PAGE_SIZE}&offset=0`);
      if(!r.ok)throw Error('Error');
      const d=await r.json();
      const fb=d.songs.map(serverToTrack);
