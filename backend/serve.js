...
// ⚠️ CONFIGURACIÓN: Tu carpeta de música (desde .env o por defecto)
require('dotenv').config();
const MUSIC_DIR = process.env.MUSIC_PATH || path.join('C:', 'Users', 'wolf', 'Music');
...
const getMusicFilesRecursive = (dir) => {
  const musicExtensions = new Set(['.mp3', '.wav', '.m4a', '.ogg', '.flac', '.aac']);
  const results = [];
  ...
  const walk = (currentDir, relativePath = '') => {
    ...
    try {
      ...
      console.warn(`⚠️ No se pudo leer carpeta: ${currentDir}`);
      return;
    }
    ...
    for (const entry of entries) {
      ...
      } else if (entry.isFile() && musicExtensions.has(path.extname(entry.name).toLowerCase())) {
        results.push({
          fullPath,
...
const syncDatabase = async () => {
  ...
  console.log("📂 Escaneando recursivamente:", MUSIC_DIR);
  
  const { parseFile } = await import('music-metadata');
  const musicFiles = getMusicFilesRecursive(MUSIC_DIR);
  
  console.log(`📊 Encontrados ${musicFiles.length} archivos de música`);

  const songsList = [];

  for (const { fullPath, relativePath, filename } of musicFiles) {
    const baseName = path.parse(relativePath).name;
    
    // Buscar imágenes en las subcarpetas de album_art / artist_art
